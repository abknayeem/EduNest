...
export const togglePublishCourse = async (req, res) => {
  try {
    ...
    if (course.isPublished) {
        try {
            const emailHtml = `
                <p>Hi ${course.creator.name},</p>
                <p>Congratulations! Your course, "<strong>${course.courseTitle}</strong>", has been successfully published and is now live on EduNest.</p>
                <p>Students can now enroll and start learning. You can view your published course here:</p>
                <a href="${process.env.FRONTEND_URL}/course-detail/${course._id}" style="display: inline-block; padding: 10px 20px; color: white; background-color: #007bff; text-decoration: none; border-radius: 5px;">View Your Course</a>
            `;
            await sendEmail({
  ...
  try {
    ...
    try {
        ...
        const emailHtml = `
            ...
            <p>We are writing to inform you that your course, "<strong>${course.courseTitle}</strong>", has been removed from the EduNest platform by an administrator.</p>
