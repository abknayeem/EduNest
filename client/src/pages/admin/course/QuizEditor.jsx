...
import { useGetQuizByCourseQuery, useSaveQuizMutation } from '@/features/api/quizApi';
import { Loader2, PlusCircle, Trash2, ArrowLeft } from 'lucide-react';
import LoadingSpinner from '@/components/LoadingSpinner';

const QuizEditor = () => {
  const { courseId } = useParams();
  const { data: quizData, isLoading: isQuizLoading } = useGetQuizByCourseQuery(courseId);
  ...
  const handleSaveQuiz = async () => {
    if (questions.some(q => !q.questionText || q.options.some(o => !o) || !q.correctAnswer)) {
  ...
  return (
    <div>
        <Link to={`/admin/course/${courseId}`} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-900 mb-4">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Create and manage the quiz for this course. Click "Save Quiz" when you are done.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                    ...
                    <Input id="time-limit" type="number" value={timeLimit} onChange={(e) => setTimeLimit(e.target.value)} placeholder="0 for no limit" />
                </div>
                <div className="space-y-2">
                    <Label htmlFor="passing-score">Passing Score (%)</Label>
                    <Input id="passing-score" type="number" value={passingScore} onChange={(e) => setPassingScore(e.target.value)} placeholder="e.g., 70" />
                </div>
            </div>
            ...
            {questions.map((q, qIndex) => (
              <Card key={qIndex} className="p-4 bg-gray-50 dark:bg-gray-900/50">
                <CardHeader className="flex flex-row items-center justify-between p-2">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => handleRemoveQuestion(qIndex)}>
                ...
                <CardContent className="space-y-4 p-2">
                    ...
                    <div>
                        ...
                        <RadioGroup value={q.correctAnswer} onValueChange={(value) => handleQuestionChange(qIndex, 'correctAnswer', value)} className="mt-2 space-y-2">
